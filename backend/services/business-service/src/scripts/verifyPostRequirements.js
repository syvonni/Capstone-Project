...
mongoose.connect(mongoUri).then(async () => {
  ...
  try {
    ...
    const withCustomFields = await PostRequirement.countDocuments({ customFields: { $exists: true, $ne: [] } });
    
    console.log(`Total post requirements: ${count}`);
    console.log(`With custom fields: ${withCustomFields}`);
    
    // Show a few examples
    const examples = await PostRequirement.find({ customFields: { $exists: true, $ne: [] } }).limit(3);
