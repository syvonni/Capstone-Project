...
router.get("/", requireJwt, async (req, res) => {
  try {
    ...
    return respondOk(res, 200, brackets);
  } catch (err) {
    console.error("GET /admin/tax-brackets error:", err);
    return respondError(res, 500, "INTERNAL", "Failed to fetch tax brackets");
  }
});
...
router.get("/:id", requireJwt, async (req, res) => {
  try {
    ...
    if (!bracket) {
      return respondError(res, 404, "NOT_FOUND", "Tax bracket not found");
    }
    return respondOk(res, 200, bracket);
  } catch (err) {
    console.error("GET /admin/tax-brackets/:id error:", err);
    return respondError(res, 500, "INTERNAL", "Failed to fetch tax bracket");
  }
});
  ...
  async (req, res) => {
    try {
      ...
      if (!lobId || !taxBasis || !name || minValue == null) {
        return respondError(res, 400, "VALIDATION_ERROR", "lobId, taxBasis, name, and minValue are required");
      }

      if (excessRate && !excessRateType) {
        return respondError(res, 400, "VALIDATION_ERROR", "excessRateType is required when excessRate is provided");
      }

      // Validate that the LOB exists
      const lob = await Lob.findById(lobId);
      if (!lob) {
        return respondError(res, 400, "VALIDATION_ERROR", "Line of Business not found");
      }
      ...
      return respondOk(res, 201, bracket);
    } catch (err) {
      console.error("POST /admin/tax-brackets error:", err);
      return respondError(res, 500, "INTERNAL", "Failed to create tax bracket");
    }
  },
);
...
router.put("/:id", requireJwt, requireRole(["admin"]), async (req, res) => {
  try {
    ...
    return respondError(res, 500, "INTERNAL", "Failed to update tax bracket");
  }
});
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      return respondOk(res, 200, { disabled: true });
    } catch (err) {
      console.error("DELETE /admin/tax-brackets/:id error:", err);
      return respondError(res, 500, "INTERNAL", "Failed to disable tax bracket");
    }
  },
);
