...
export default function LobOverview({ lob, initialValues, variables, documents, postRequirements, taxBrackets, token, loading = false }) {
  ...
  const selectedVariableFeeRules = variables.filter(rule => variableFeeRuleIds.includes(rule._id))
  ...
  const requiredPostRequirementIds = requiredPostRequirementValues.map(r => typeof r === 'object' ? r._id : r)
  const conditionalPostRequirementIds = conditionalPostRequirementValues.map(r => typeof r === 'object' ? r._id : r)
  const requiredPostRequirements = postRequirements.filter(pr => requiredPostRequirementIds.includes(pr._id))
  const conditionalPostRequirements = postRequirements.filter(pr => conditionalPostRequirementIds.includes(pr._id))

  const categoryMapping = LINE_OF_BUSINESS_BY_TAX_CODE[initialValues.category]
  const categoryLabel = categoryMapping
    ? categoryMapping.lineOfBusiness.charAt(0).toUpperCase() + categoryMapping.lineOfBusiness.slice(1).replace('_', ' ')
      ...
      <InfoGrid
        ...
        items={[
          ...
          { label: 'Description', value: initialValues.description || '-' },
          {
            ...
            title: 'Capital Tax',
            items: (() => {
              const capitalizationBrackets = taxBrackets.filter(tb => tb.taxBasis === 'capitalization')
              return capitalizationBrackets.length > 0 ? capitalizationBrackets.map((bracket) => {
          ...
          },
          {
            ...
            items: (() => {
              ...
              return grossSalesBrackets.length > 0 ? grossSalesBrackets.map((bracket) => {
                const range = bracket.maxValue !== undefined && bracket.maxValue !== null
                  ? `₱${bracket.minValue?.toLocaleString() || '0'} to ₱${bracket.maxValue?.toLocaleString()}`
                  : `₱${bracket.minValue?.toLocaleString() || '0'}+`
                const amount = bracket.fixedAmount !== null && bracket.fixedAmount !== undefined
          ...
          { type: 'divider' },
          {
            ...
            items: selectedDocuments.length > 0 ? selectedDocuments.map((doc) => ({
              text: doc.name,
          ...
          },
          {
            ...
            items: conditionalPostRequirements.length > 0 ? conditionalPostRequirements.map((req) => ({
              text: req.name,
          ...
          },
          {
            ...
            items: requiredPostRequirements.length > 0 ? requiredPostRequirements.map((req) => ({
              text: req.name,
              to: `/admin/post-requirements?selectedId=${req._id}`,
            })) : [{ text: '-' }],
          },
          {
            type: 'sublist',
            title: 'Variables',
            items: selectedVariableFeeRules.length > 0 ? selectedVariableFeeRules.map((rule) => ({
              text: rule.name,
