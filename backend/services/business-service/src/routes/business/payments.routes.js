...
router.get("/", requireJwt, async (req, res) => {
  try {
    ...
    return respondOk(res, 200, payments);
  } catch (err) {
    console.error("GET /payments error:", err);
    return respondError(res, 500, "INTERNAL", "Failed to fetch payments");
  }
});
...
router.get("/pending", requireJwt, async (req, res) => {
  try {
    ...
    return respondOk(res, 200, enriched);
  } catch (err) {
    console.error("GET /payments/pending error:", err);
    return respondError(res, 500, "INTERNAL", "Failed to fetch pending payments");
  }
});
...
router.get("/history", requireJwt, async (req, res) => {
  try {
    ...
    return respondError(res, 500, "INTERNAL", "Failed to fetch payment history");
  }
});
...
router.get("/:paymentId", requireJwt, async (req, res) => {
  try {
    ...
    if (!payment) {
      return respondError(res, 404, "NOT_FOUND", "Payment not found");
    }

    return respondOk(res, 200, payment);
  } catch (err) {
    console.error("GET /payments/:paymentId error:", err);
    return respondError(res, 500, "INTERNAL", "Failed to fetch payment");
  }
});
...
router.post("/", requireJwt, async (req, res) => {
  try {
    ...
    if (!businessId || !paymentType || !amount) {
      return respondError(res, 400, "VALIDATION_ERROR", "businessId, paymentType, and amount are required");
    }

    if (amount <= 0) {
      return respondError(res, 400, "VALIDATION_ERROR", "Amount must be greater than 0");
    }
    ...
    if (!business) {
      return respondError(res, 404, "BUSINESS_NOT_FOUND", "Business not found");
    }
    ...
    return respondOk(res, 201, payment);
  } catch (err) {
    ...
    if (err.code === 11000 || err.message?.includes("E11000")) {
      ...
      return respondError(res, 409, "DUPLICATE", "Payment already exists for this business and payment type");
    }
    console.error("POST /payments error:", err);
    return respondError(res, 500, "INTERNAL", "Failed to create payment");
  }
});
...
router.post("/:paymentId/pay", requireJwt, async (req, res) => {
  try {
    ...
    if (!paymentMethod) {
      return respondError(res, 400, "VALIDATION_ERROR", "paymentMethod is required");
    }
    ...
    if (payment.status === "paid") {
      return respondError(res, 400, "ALREADY_PAID", "Payment has already been processed");
    }

    if (payment.status === "cancelled") {
      return respondError(res, 400, "CANCELLED", "Payment has been cancelled");
    }
    ...
    return respondError(res, 500, "INTERNAL", "Failed to process payment");
  }
});
...
router.put("/:paymentId/cancel", requireJwt, async (req, res) => {
  try {
    ...
    if (payment.status !== "pending") {
      return respondError(res, 400, "INVALID_STATUS", "Only pending payments can be cancelled");
    }
    ...
    return respondError(res, 500, "INTERNAL", "Failed to cancel payment");
  }
});
...
router.post("/:paymentId/receipt", requireJwt, async (req, res) => {
  try {
    ...
    if (payment.status !== "paid") {
      return respondError(res, 400, "NOT_PAID", "Receipt can only be generated for paid payments");
    }
    ...
    return respondError(res, 500, "INTERNAL", "Failed to generate receipt");
  }
});
...
router.post("/mock", requireJwt, async (req, res) => {
  try {
    ...
    if (!businessId || !amount) {
      return respondError(res, 400, "VALIDATION_ERROR", "businessId and amount are required");
    }
    ...
    if (err.code === 11000 || err.message?.includes("E11000")) {
      ...
      return respondError(res, 409, "DUPLICATE", "Payment already exists for this business");
    }
    console.error("POST /payments/mock error:", err);
    return respondError(res, 500, "INTERNAL", "Failed to create mock payment");
  }
});
