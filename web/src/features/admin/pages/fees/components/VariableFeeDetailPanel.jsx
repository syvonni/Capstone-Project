...
export default function VariableFeeDetailPanel({ ruleId, rule, onSave, _categoryOptions = [] }) {
  ...
  }), [rule?.name, rule?.notes, rule?.description, rule?.question, rule?.calculationMethod, rule?.customCalculationMethod, rule?.baseRate, rule?.unit, rule?.brackets, rule?.classifications, rule?.isActive])
  ...
  const handleSave = async () => {
    try {
      ...
      const payload = {
        ...
        customCalculationMethod: values.calculationMethod === 'custom' ? values.customCalculationMethod : null,
        baseRate: (values.calculationMethod === 'bracketed' || values.calculationMethod === 'classification') ? null : values.baseRate,
        unit: values.unit,
        categories: values.categories,
        brackets: values.calculationMethod === 'bracketed' ? values.brackets : [],
        classifications: values.calculationMethod === 'classification' ? values.classifications : [],
      ...
      await runWithStepUp(async (stepUpToken) => {
        console.log('Step-up token received, calling API')
        if (isNew) {
