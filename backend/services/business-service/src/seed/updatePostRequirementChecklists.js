  ...
  try {
    ...
    for (const pr of postRequirements) {
      ...
      if (checklistId && pr.checklistId?.toString() !== checklistId) {
        ...
        await pr.save();
        console.log(`Updated PostRequirement: ${pr.code} with checklistId: ${checklistId}`);
        updatedCount++;
      }
    }

    console.log(`Updated ${updatedCount} post requirements with checklist associations`);
