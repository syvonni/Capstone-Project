...
export default function DocumentDetailPanel({ documentId, document, onSave, onDelete: _onDelete }) {
  ...
  const summaryFormatters = useMemo(() => {
    ...
    return {
      formIds: joinNames(formDefinitions),
      checklistId: nameOf(checklists),
    };
  }, [formDefinitions, checklists]);

  const summaryFieldLabels = useMemo(
    () => ({
      formIds: 'Associated Forms',
  ...
  const {
    ...
    refresh,
  } = useAudit('document', documentId, historyModalOpen && documentId !== 'new');
  ...
  const initialValues = useMemo(
    () => ({
      ...
      checklistId:
        document?.checklistId?._id?.toString() || document?.checklistId?.toString() || undefined,
    }),
    [document]
  );
  ...
  const handleConfirmPublish = async () => {
    try {
      ...
      if (
        ...
        // User cancelled the passkey/step-up prompt; no error UI needed
      } else {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <AuditHistoryModal
        ...
        eventDescriptions={AUDIT_EVENT_INFO.filter(
          (e) => e.event.startsWith('document_') && !e.event.startsWith('document_group_')
        )}
      />
      <ChangesSummaryModal
