...
const DOCUMENT_INSPECTION_ITEMS = [
  // Fire Safety Inspection Certificate
  {
    ...
    question: 'Does the establishment have a valid Fire Safety Inspection Certificate?',
    notes: 'Check for BFP-issued fire safety inspection certificate. Verify it is on file and accessible.',
  ...
  },
  {
    ...
    question: 'Is the Fire Safety Inspection Certificate valid and not expired?',
  ...
  },
  {
    ...
    question: 'Is the Fire Safety Inspection Certificate authentic with proper BFP signature and seal?',
    notes: 'Verify the certificate has proper BFP signature, official seal, and valid certificate number.',
  ...
  // Sanitary Permit
  {
    ...
    notes: 'Check for health department-issued sanitary permit. Verify it is on file and accessible.',
  ...
  },
  {
    ...
    question: 'Is the Sanitary Permit authentic with proper health office signature and seal?',
    notes: 'Verify the permit has proper health office signature, official seal, and valid permit number.',
  ...
  // Zoning Clearance
  {
    ...
    notes: 'Check for HLURB or local zoning office-issued zoning clearance. Verify it is on file and accessible.',
  ...
  },
  {
    ...
    question: 'Is the Zoning Clearance authentic with proper zoning office signature and seal?',
    notes: 'Verify the clearance has proper zoning office signature, official seal, and valid clearance number.',
...
async function seedDocumentInspectionItems() {
  ...
  for (const itemData of DOCUMENT_INSPECTION_ITEMS) {
    const existing = await InspectionItem.findOne({ customId: itemData.customId });

    // Find violation by code
    const violation = await Violation.findOne({ code: itemData.violationCode });

    if (!violation) {
      console.warn(`  ! Warning: Violation not found: ${itemData.violationCode} - skipping inspection item: ${itemData.name}`);
      continue;
    }
    ...
    if (!existing) {
      ...
      totalCreated++;
      console.log(`  + Created: ${itemData.name} (linked to violation: ${itemData.violationCode})`);
    } else {
      ...
      totalUpdated++;
      console.log(`  ~ Updated: ${itemData.name} (linked to violation: ${itemData.violationCode})`);
    }
  }
