...
const CHECKLISTS = [
  {
    ...
    description: 'Comprehensive fire safety checklist for commercial establishments',
    ...
    legalBasis: [
      {
        ...
        description: 'The Fire Code of the Philippines mandates regular fire safety inspections for commercial establishments'
      }
    ],
    items: [
      { inspectionItemName: 'Presence of Boxes Blocking Fire Exit', order: 1 },
      { inspectionItemName: 'Presence of Furniture Blocking Fire Exit', order: 2 },
      { inspectionItemName: 'Presence of Equipment Blocking Fire Exit', order: 3 },
      { inspectionItemName: 'Presence of Fire Extinguisher in Kitchen', order: 4 },
      { inspectionItemName: 'Presence of Fire Extinguisher in Storage', order: 5 },
      { inspectionItemName: 'Presence of Fire Extinguisher Near Exits', order: 6 },
    ]
  },
  {
    ...
    legalBasis: [
      {
        ...
        description: 'Prescribes minimum requirements for sanitation in food establishments'
      }
    ],
    items: [
  ...
  },
  {
    ...
    description: 'Building structural integrity and permit compliance checklist',
    ...
    legalBasis: [
      {
        ...
        description: 'Sets standards for building construction and structural safety'
      }
    ],
    items: [
    ...
    legalBasis: [
      {
        ...
        description: 'Mandates business permit requirements and display for all commercial establishments'
      }
    ],
    items: [
  ...
  try {
    ...
    for (const checklistData of CHECKLISTS) {
      ...
      for (const item of checklistData.items) {
        ...
        if (!inspectionItemId) {
          console.log(`Warning: No active inspection item found with name: ${item.inspectionItemName}`);
          continue;
        }
      ...
      if (itemsWithIds.length === 0) {
        console.log(`Warning: No valid inspection items found for checklist: ${checklistData.name}`);
        continue;
      }
...
async function forceReseed() {
  try {
    ...
    console.log(`Reseeded ${createdChecklists.length} checklists with ${linkedInspectionItems.length} linked inspection items`);
