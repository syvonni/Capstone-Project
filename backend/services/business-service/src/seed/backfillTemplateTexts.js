...
async function backfillTemplateTexts() {
  try {
    ...
    for (const seedData of REQUIREMENTS_SEED_DATA) {
      const existingDoc = await ClaimableDocument.findOne({ customId: seedData.customId });
      ...
      if (existingDoc.templateTexts && existingDoc.templateTexts.length > 0) {
        console.log(`Document ${seedData.name} already has templateTexts, skipping`);
        skippedCount++;
        continue;
      }
      ...
      console.log(`Updated ${seedData.name} with ${seedData.templateTexts.length} templateTexts`);
      updatedCount++;
    }
...
async function forceUpdateAccountClearance() {
  try {
    ...
    const seedData = REQUIREMENTS_SEED_DATA.find(d => d.customId === 'account-clearance');
    if (!seedData) {
      console.log('Account Clearance seed data not found');
      process.exit(1);
    }

    const existingDoc = await ClaimableDocument.findOne({ customId: 'account-clearance' });
    if (!existingDoc) {
    ...
    console.log(`Force updated Account Clearance with ${seedData.templateTexts.length} templateTexts`);
  } catch (error) {
...
async function forceUpdateAllDocuments() {
  try {
    ...
    for (const seedData of REQUIREMENTS_SEED_DATA) {
      ...
      console.log(`Force updated ${seedData.name} with ${seedData.templateTexts.length} templateTexts`);
      updatedCount++;
    }
