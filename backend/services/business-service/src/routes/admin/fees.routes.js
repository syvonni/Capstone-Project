...
router.get("/", requireJwt, async (req, res) => {
  try {
    ...
    return respondOk(res, 200, fees);
  } catch (err) {
    console.error("GET /admin/fees error:", err);
    return respondError(res, 500, "INTERNAL", "Failed to fetch fees");
  }
});
...
router.get("/:id", requireJwt, async (req, res) => {
  try {
    ...
    return respondOk(res, 200, fee);
  } catch (err) {
    console.error("GET /admin/fees/:id error:", err);
    return respondError(res, 500, "INTERNAL", "Failed to fetch fee");
  }
});
...
router.put("/:id", requireJwt, requireRole(["admin"]), async (req, res) => {
  try {
    ...
    return respondError(res, 500, "INTERNAL", "Failed to update fee");
  }
});
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      return respondError(res, 500, "INTERNAL", "Failed to disable fee");
    }
  },
);
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      return respondError(res, 500, "INTERNAL", "Failed to update variable calculation");
    }
  },
);
...
router.get("/by-category/:category", requireJwt, async (req, res) => {
  try {
    ...
    return respondError(res, 500, "INTERNAL", "Failed to fetch fees by category");
  }
});
