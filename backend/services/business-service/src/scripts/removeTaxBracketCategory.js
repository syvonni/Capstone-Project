...
mongoose.connect(mongoUri).then(async () => {
  ...
  try {
    ...
    const bracketsWithCategory = await TaxBracket.find({ category: { $exists: true } });
    
    console.log(`Found ${bracketsWithCategory.length} tax brackets with category field`);

    if (bracketsWithCategory.length === 0) {
      console.log('No tax brackets with category field found. Migration complete.');
    ...
    console.log(`Successfully removed category field from ${results.length} tax brackets`);
    
    // Verify the changes
    const remainingWithCategory = await TaxBracket.countDocuments({ category: { $exists: true } });
    console.log(`Tax brackets still with category field: ${remainingWithCategory}`);
