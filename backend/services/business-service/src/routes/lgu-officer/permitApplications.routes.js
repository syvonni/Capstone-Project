...
async function sendApplicationEmail(application, emailType, metadata = {}) {
  ...
  try {
    ...
    const mailer = require("../../../../auth-service/s../../lib/mailer");
  ...
  async (req, res) => {
    try {
      ...
      if (application.reviewedBy) {
        return respondError(res, 400, "already_claimed", "Application already claimed");
      }
      ...
      return respondError(res, 500, "claim_error", "Failed to claim application");
    }
  },
);
  ...
  async (req, res) => {
    try {
      ...
      if (String(application.reviewedBy) !== String(officerId)) {
        return respondError(res, 403, "forbidden", "You can only review your own claimed applications");
      }
      ...
      if (decision === "approve") {
        ...
        if (!businessProfile) {
          return respondError(res, 404, "profile_not_found", "Business profile not found");
        }
        ...
        return respondOk(res, 200, { application, business });
        ...
        return respondOk(res, 200, { application });
        ...
        return respondError(res, 400, "invalid_decision", "Invalid decision");
      }
    } catch (err) {
      console.error(
      ...
      return respondError(res, 500, "review_error", "Failed to review application");
    }
  },
);
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      return respondError(res, 500, "fetch_error", "Failed to fetch applications");
    }
  },
);
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      return respondError(res, 500, "fetch_error", "Failed to fetch application");
    }
  },
);
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      if (!updated) {
        return respondError(res, 409, "conflict", "Application already claimed by another officer");
      }
      ...
      return respondError(res, 500, "claim_failed", "Failed to claim application");
    }
  },
);
  ...
  async (req, res) => {
    try {
      ...
      if (
        ...
        return respondError(res, 403, "forbidden", "Only the claiming officer can release this application");
      }
      ...
      return respondError(res, 500, "release_failed", "Failed to release application");
    }
  },
);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!newStatus) {
        return respondError(res, 400, "validation_error", "New status is required");
      }
      ...
      return respondError(res, 500, "reset_error", err.message || "Failed to reset application status");
    }
  },
);
  ...
  async (req, res) => {
    try {
      ...
      if (!payload || payload.length === 0) {
        return respondError(res, 400, "missing_data", "fieldKey and status, or decisions array, required");
      }
      ...
      return respondError(res, 500, "update_error", "Failed to update field decisions");
    }
  },
);
  ...
  async (req, res) => {
    try {
      ...
      if (
        ...
        return respondError(res, 400, "invalid_data", "actionType must be one of: complete_review, reject, return, reject_appeal");
      }
      ...
      if (doc.pendingAction?.actionType) {
        return respondError(res, 409, "conflict", "A pending action already exists. Cancel it first.");
      }
      ...
      return respondError(res, 500, "server_error", err.message || "Failed to create pending action");
    }
  },
);
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      return respondError(res, 500, "server_error", err.message || "Failed to cancel pending action");
    }
  },
);
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      return respondOk(res, 200, { pendingAction: doc.pendingAction });
    } catch (err) {
      console.error("GET /pending-action error:", err);
      return respondError(res, 500, "server_error", err.message || "Failed to get pending action");
    }
  },
);
  ...
  async (req, res) => {
    try {
      ...
      if (!pendingAction || !pendingAction.actionType) {
        return respondError(res, 400, "no_pending_action", "No pending action to execute");
      }
      ...
      return respondError(res, 500, "server_error", err.message || "Failed to execute pending action");
    }
  },
);
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      const officerId = req._userId || req.user?.id;
      if (String(application.reviewedBy) !== String(officerId)) {
        return respondError(res, 403, "forbidden", "You can only edit your own claimed drafts");
      }
      ...
      return respondError(res, 500, "update_error", "Failed to update form data");
    }
  },
);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!permitType) {
        return respondError(res, 400, "missing_permit_type", "permitType is required");
      }

      // Verify business owner exists
      const businessOwner = await User.findById(ownerId);
      if (!businessOwner) {
        return respondError(res, 404, "owner_not_found", "Business owner not found");
      }
      ...
      const PermitForm = require('../../admin-service/s../../models/PermitForm')
      const permitForm = await PermitForm.findOne({ formType: permitType, isActive: true })
      if (!permitForm) {
        return respondError(res, 404, "no_permit_form", "No active permit form found for this permit type");
      }
      ...
      return respondOk(res, 201, { application });
    } catch (err) {
      console.error("POST /api/lgu-officer/walk-in-applications error:", err);
      if (err.response?.status === 404) {
        return respondError(res, 404, "form_not_found", "Form definition not found");
      }
      return respondError(res, 500, "create_error", "Failed to create walk-in application");
    }
  },
);
  ...
  async (req, res) => {
    try {
      ...
      if (application.applicationStatus !== "officer_draft") {
        return respondError(res, 400, "invalid_status", "Only officer draft applications can be finished");
      }

      if (String(application.reviewedBy) !== String(officerId)) {
        return respondError(res, 403, "forbidden", "You can only finish your own claimed drafts");
      }
      ...
      if (
        ...
        return respondError(res, 400, "form_incomplete", "Application form must be completed before finishing");
      }
      ...
      return respondError(res, 500, "finish_error", "Failed to finish application");
    }
  },
);
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (
        ...
        return respondError(res, 400, "invalid_email_type", "Invalid email type");
      }
      ...
      if (!isAdmin && !isClaimedByOfficer) {
        return respondError(res, 403, "not_claimed", "You must claim this application before resending emails");
      }
      ...
      if (lockUntil && new Date(lockUntil) > now) {
        return respondError(res, 429, "locked", "Email resend is in progress, please wait");
      }
      ...
      try {
        ...
        return respondError(res, 500, "email_send_failed", `Failed to send email: ${emailErr.message}`);
      }
    } catch (err) {
      console.error(
      ...
      return respondError(res, 500, "resend_error", "Failed to resend email");
    }
  },
);
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      return respondError(res, 500, "reset_error", "Failed to reset email status");
    }
  },
);
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      if (application.applicationStatus !== "officer_draft") {
        return respondError(res, 400, "invalid_status", "Only officer draft applications can be deleted");
      }

      if (String(application.reviewedBy) !== String(officerId)) {
        return respondError(res, 403, "forbidden", "You can only delete your own drafts");
      }
      ...
      return respondError(res, 500, "delete_error", "Failed to delete application");
    }
  },
);
