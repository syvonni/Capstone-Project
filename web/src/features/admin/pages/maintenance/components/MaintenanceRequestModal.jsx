...
export default function MaintenanceRequestModal({
  ...
  const whenToStart =
    Form.useWatch('whenToStart', form) || (forceScheduleMode ? 'scheduled' : undefined);
  ...
  React.useEffect(() => {
    ...
    async function fetchMaintenanceStatus() {
      if (!open) return;
      try {
        const maintenance = await get('/api/maintenance/status', { skipAuth: true }).catch(() => ({
          active: false,
          scheduled: false,
        }));
        if (!alive) return;
      ...
      {isEndMaintenanceFlow ? (
        <>
          <div style={{ marginTop: 8, marginBottom: 16 }}>
            <Text type="secondary">
              Submit a request to end maintenance mode. The request will require approval from two
              admins before the system returns to normal operation.
            </Text>
          </div>
          <Form
            ...
            form={form}
            layout="vertical"
            initialValues={{ action: 'disable', reasonPreset: 'completed' }}
            requiredMark={false}
          >
            ...
            <Form.Item
              ...
              label={
                <span>
                  Reason<span style={{ color: token.colorError, marginLeft: 4 }}>*</span>
                </span>
              }
            ...
            {reasonPreset === REASON_PRESET_OTHER && (
              <Form.Item
                ...
                label={
                  <span>
                    Custom reason<span style={{ color: token.colorError, marginLeft: 4 }}>*</span>
                  </span>
                }
                rules={[
            ...
            <Form.Item
              ...
              label={
                <span>
                  Message<span style={{ color: token.colorError, marginLeft: 4 }}>*</span>
                </span>
              }
              rules={[
                        ...
                        validator: (_, value) => {
                          ...
                          if (value.isAfter(maxDate)) {
                            return Promise.reject(
                              new Error(
                                `Maintenance cannot be scheduled more than ${MAX_SCHEDULING_HORIZON_DAYS} days in advance`
                              )
                            );
                          }
                          ...
                          if (maintenanceActive && currentMaintenance?.expectedResumeAt) {
                            ...
                            if (value.isBefore(currentResumeAt)) {
                              return Promise.reject(
                                new Error(
                                  'Start time must be after the current maintenance session ends'
                                )
                              );
                            }
                          }
                          if (expectedResumeAt && hasConflict(value, expectedResumeAt)) {
                            return Promise.reject(
                              new Error(
                                'Selected schedule overlaps with an existing pending or approved maintenance request. Please choose another time slot.'
                              )
                            );
                          }
                          ...
                          if (expectedResumeAt) {
                            ...
                            if (durationHours < MIN_MAINTENANCE_DURATION_HOURS) {
                              return Promise.reject(
                                new Error(
                                  `Maintenance duration must be at least ${MIN_MAINTENANCE_DURATION_HOURS} hour(s)`
                                )
                              );
                            }
                            ...
                            if (durationDays > MAX_MAINTENANCE_DURATION_DAYS) {
                              return Promise.reject(
                                new Error(
                                  `Maintenance duration cannot exceed ${MAX_MAINTENANCE_DURATION_DAYS} day(s)`
                                )
                              );
                            }
                          }
              ...
              rules={[
                { required: true, message: 'Select expected resume time' },
                {
                  validator: (_, value) => {
                    ...
                    const startValue =
                      isScheduled || forceScheduleMode ? scheduledStartAt : dayjs();
                    ...
                    if (maintenanceActive && currentMaintenance?.expectedResumeAt) {
                      ...
                      if (value.isBefore(currentResumeAt)) {
                        return Promise.reject(
                          new Error(
                            'Resume time must be after the current maintenance session ends'
                          )
                        );
                      }
                    }
            ...
            >
              <DatePicker
                ...
                disabledDate={(d) => {
                  ...
                  const now = dayjs();
                  const maxDate = dayjs()
                    .add(MAX_SCHEDULING_HORIZON_DAYS, 'day')
                    .add(MAX_MAINTENANCE_DURATION_DAYS, 'day');
                  if (d.isBefore(now, 'day') || d.isAfter(maxDate)) return true;
                  if (maintenanceActive && currentMaintenance?.expectedResumeAt) {
                    const currentResumeAt = dayjs(currentMaintenance.expectedResumeAt);
                    if (d.isBefore(currentResumeAt, 'day')) return true;
