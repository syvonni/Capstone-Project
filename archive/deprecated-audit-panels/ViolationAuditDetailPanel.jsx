...
export default function ViolationAuditDetailPanel({ audit }) {
  if (!audit) {
    return (
  ...
  return (
    <div style={{ padding: 16 }}>
      <Descriptions column={1} size="small" bordered>
        ...
        {metadata.description && <Descriptions.Item label="Description">{metadata.description}</Descriptions.Item>}
        {metadata.notes && <Descriptions.Item label="Notes">{metadata.notes}</Descriptions.Item>}
        {metadata.severity && <Descriptions.Item label="Severity">{metadata.severity}</Descriptions.Item>}
        {metadata.correctiveAction && <Descriptions.Item label="Corrective Action">{metadata.correctiveAction}</Descriptions.Item>}
