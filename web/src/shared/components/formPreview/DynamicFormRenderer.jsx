...
import { filterSectionsByFormValues } from '@/features/business-owner/utils/formUtils'
import {
  Input,
  Select,
  DatePicker,
...
function DynamicField({ field, form, token, readOnly, businessId, onDocumentCid, onSaveDraft, documents = {}, revisionFieldKeys, fieldReviewDecisions }) {
  ...
  // Shared Form.Item style: highlights the field when the reviewer requested a change
  const requestChangeStyle = hasRequestChange || hasAddressRequestChange
    ? { border: `1px dashed ${token.colorVolcano}`, padding: '12px', borderRadius: '8px', marginBottom: '16px' }
    : { marginBottom: '16px' }
  ...
  switch (field.type) {
    ...
    case 'email':
      return (
        ...
        >
          <Input type="email" placeholder={field.placeholder || ''} disabled={effectiveReadOnly} readOnly={effectiveReadOnly} />
        </Form.Item>
      )
    ...
    case 'phone':
      return (
        <Form.Item
          ...
          style={requestChangeStyle}
        >
          <Input type="tel" placeholder={field.placeholder || ''} disabled={effectiveReadOnly} readOnly={effectiveReadOnly} />
        </Form.Item>
      )
...
function renderSectionContent(section, sIdx, form, token, isMobile, readOnly, businessId, onDocumentCid, onSaveDraft, documents, revisionFieldKeys, fieldReviewDecisions, renderedFieldKeys) {
  // Line-of-business sections are rendered by their own component rather than as a field list
  if (section.type === 'lob_section') {
    return <LobSection key={sIdx} isEditMode={!readOnly} />
  }
          ...
          if (item.type === 'address' || item.type === 'address_alaminos') {
            return (
              <Col key={`${sIdx}-${fieldKey}`} span={24}>
                <DynamicField field={item} form={form} token={token} readOnly={readOnly} businessId={businessId} onDocumentCid={onDocumentCid} onSaveDraft={onSaveDraft} documents={documents} revisionFieldKeys={revisionFieldKeys} fieldReviewDecisions={fieldReviewDecisions} />
              </Col>
            )
          }
          return (
 ...
 *
 * `mode` controls which capabilities are active:
 *  - 'application' (default): live application. Real document uploads, revision-mode
 *    field locking, and reviewer "requested change" decoration are all enabled.
 *  - 'preview': static preview of a form definition (e.g. the admin form builder).
 *    Application-review and submission concerns are disabled — uploads are inert and
 *    revision/review props are ignored even if passed.
 */
export default function DynamicFormRenderer({
  ...
  // In preview mode, strip the application-only wiring at this single choke point so
  // no downstream field has to know about the mode. Nulling businessId/onDocumentCid
  // is what makes uploads inert, since `canUpload` requires both.
  const isPreview = mode === 'preview'
  const activeBusinessId = isPreview ? null : businessId
  const activeOnDocumentCid = isPreview ? null : onDocumentCid
  const activeOnSaveDraft = isPreview ? null : onSaveDraft
  const activeRevisionFieldKeys = isPreview ? null : revisionFieldKeys
  const activeFieldReviewDecisions = isPreview ? undefined : fieldReviewDecisions
    ...
    return (
      <div>
        ...
        {visibleSections.map((sec, sIdx) => (
          ...
          >
            {renderSectionContent(sec, sIdx, form, token, isMobile, readOnly, activeBusinessId, activeOnDocumentCid, activeOnSaveDraft, documents, activeRevisionFieldKeys, activeFieldReviewDecisions, renderedFieldKeys)}
          </div>
        ))}
      </div>
    )
    ...
    <div>
      {visibleSections.map((section, sIdx) => (
        ...
        >
          ...
          {renderSectionContent(section, sIdx, form, token, isMobile, readOnly, activeBusinessId, activeOnDocumentCid, activeOnSaveDraft, documents, activeRevisionFieldKeys, activeFieldReviewDecisions, renderedFieldKeys)}
        </Card>
      ))}
    </div>
  )
