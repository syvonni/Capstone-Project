...
export default function LOBDashboardCard() {
  ...
  const { issues: dataQualityIssues } = useDataQuality('lob')
  
  // Use performance hook to fetch metrics
  const { metrics: performanceMetrics } = usePerformance('lob')
  ...
  const statusLinks = useMemo(() => {
    const activeLobs = lobs.filter(l => l.isActive).length
    const disabledLobs = lobs.filter(l => !l.isActive).length
  ...
  const issuesLink = useMemo(() => {
    ...
    // Build modal content with grouped issue types and their LOBs
