...
export default function LobConfiguration({ form, handleFormValuesChange, variables, documents, postRequirements }) {

  const variableFeeOptions = variables.map(rule => ({
    value: rule._id,
    label: rule.calculationMethod === 'classification'
      ...
      >
        ...
        >
          <Select
            ...
            options={postRequirements.map(req => ({ value: req._id, label: req.name, question: req.question }))}
            style={{ width: '100%' }}
          />
        </Form.Item>
      </Form>
