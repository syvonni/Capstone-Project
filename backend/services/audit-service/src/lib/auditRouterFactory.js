...
function createAuditRouter(entityType, config, options = {}) {
  ...
  // }
  router.get(
    `${singularPrefix ? "/" + singularPrefix : ""}/:${singularPath}`,
    requireJwt,
    requireRole(config.permissions),
    async (req, res) => {
      try {
        ...
        return handleAuditError(err, res, logger, {
          entityType,
          endpoint: "singular",
        });
      }
  ...
  // }
  router.get(
    `${globalPrefix ? "/" + globalPrefix : ""}/${globalPath}`,
    requireJwt,
    requireRole(config.permissions),
    async (req, res) => {
      try {
