...
} from '@/features/authentication/utils/validations/changePasswordRules.js';
...
const LABELS = {
  admin: {
    ...
    welcomeMessage:
      "To keep your account secure, we'll guide you through a quick setup. You'll set a new password and enable two-factor authentication (MFA) to protect your administrative access.",
  ...
  staff: {
    ...
    welcomeMessage:
      "To keep your account secure, we'll guide you through a quick setup. You'll set a new password and enable two-factor authentication (MFA) to protect your staff access.",
  ...
  business_owner: {
    ...
    welcomeMessage:
      "To keep your account secure, we'll guide you through a quick setup. You can enable two-factor authentication (MFA) to protect your business account.",
...
export default function OnboardingStepContent({
  ...
  const stepsItems =
    mode === 'password-expired'
      ? [{ title: 'Password' }, { title: 'Complete' }]
      : mode === 'mfa-only'
        ? [{ title: 'MFA' }, { title: 'Complete' }]
        : variant === 'admin' || variant === 'staff'
          ? [{ title: 'Welcome' }, { title: 'Password' }, { title: 'MFA' }, { title: 'Complete' }]
          : mustChange
            ? [{ title: 'Welcome' }, { title: 'Password' }, { title: 'MFA' }, { title: 'Complete' }]
            : [{ title: 'Welcome' }, { title: 'MFA' }, { title: 'Complete' }];
  ...
  const stepContent = (() => {
    ...
    if (
      (currentStep === 1 &&
        mode === 'onboarding' &&
        (variant === 'admin' || variant === 'staff' || mustChange)) ||
      (currentStep === 1 && mode === 'password-expired')
    ) {
      return (
        <>
          <Title level={4}>
            {mode === 'password-expired' ? 'Change Your Password' : 'Set Your Own Password'}
          </Title>
    ...
    if (
      (currentStep === 2 &&
        mode === 'onboarding' &&
        (mustChange || variant === 'admin' || variant === 'staff')) ||
      (currentStep === 1 &&
        mode === 'onboarding' &&
        !mustChange &&
        variant !== 'admin' &&
        variant !== 'staff') ||
      (currentStep === 0 && mode === 'mfa-only')
    ) {
      if (checkingMfa) {
        return (
          <>
    ...
    if (
      ...
      (currentStep === 2 &&
        ...
        variant !== 'admin' &&
        variant !== 'staff') ||
      (currentStep === 2 && mode === 'password-expired') ||
      (currentStep === 1 && mode === 'mfa-only')
    ) {
      return (
        <>
          <div style={{ padding: '24px 0' }}>
            ...
            <Title level={4}>
              {mode === 'password-expired' ? 'Password Updated!' : 'Setup Complete!'}
            </Title>
    ...
    return (
      <div style={{ padding: '24px 0' }}>
        ...
        <Paragraph type="secondary" style={{ textAlign: 'center' }}>
          Unable to render step {currentStep} in mode {mode} (mustChange={String(mustChange)},
          variant={variant})
        </Paragraph>
  ...
  return (
    <div style={{ maxWidth: 400, margin: '0 auto', textAlign: 'center', padding: '24px 0' }}>
      {mode !== 'password-expired' &&
        mode !== 'mfa-only' &&
        currentStep > 0 &&
        currentStep < (variant === 'admin' || variant === 'staff' ? 3 : mustChange ? 3 : 2) && (
          <Steps
            type="inline"
            current={currentStep}
            items={stepsItems}
