...
describe("documentResolver", () => {
  ...
  describe("formatFieldValue", () => {
    it("should format address objects", () => {
      ...
      expect(formatFieldValue(address, "address")).toBe(
        "Unit 123, Main St, Barangay 1, City, Province, 1234",
      );
    });
  ...
  describe("resolveTemplateTexts", () => {
      ...
      const templateTexts = [
        {
          ...
          bindings: [
            { formId: "form-1", sectionIndex: 0, fieldKey: "businessName" },
          ],
        },
      ];
      ...
      const templateTexts = [
        {
          ...
          sourceType: "form_field",
          bindings: [
            { formId: "form-1", sectionIndex: 0, fieldKey: "nonExistent" },
          ],
        },
      ];
    ...
    it("should handle unknown source types", () => {
      ...
      expect(
        warnings.some((w) => w.reason.includes("Unknown sourceType")),
      ).toBe(true);
    });
  });
  ...
  describe("renderDocumentHtml", () => {
    it("should replace placeholders with resolved values", () => {
      const templateHtml =
        "<div>Business: {{businessName}}</div><div>Owner: {{ownerName}}</div>";
